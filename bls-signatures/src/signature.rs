...
impl SignatureProjective {
    ...
    /// Verify a list of signatures against a message and a list of public keys,
    /// identifying which signatures are valid using a sequential binary search.
    #[cfg(feature = "parallel")]
    pub fn seq_verify_batch_binary_search<
    ...
    >(
        ...
        Self::sequential_recursive_identify(&inputs, message, &mut results);

        Ok(results)
    }

    /// Helper function for the sequential binary search.
    #[cfg(feature = "parallel")]
    #[allow(clippy::arithmetic_side_effects)]
    fn sequential_recursive_identify(
        inputs: &[(PubkeyProjective, SignatureProjective)],
        message: &[u8],
        results: &mut [bool],
    ) {
        ...
        // Sequential aggregation
        let (aggregate_pubkey, aggregate_signature) = inputs.iter().fold(
            (
                PubkeyProjective::identity(),
                SignatureProjective::identity(),
            ),
        ...
        if is_valid {
            ...
            Self::sequential_recursive_identify(left_inputs, message, left_results);
            Self::sequential_recursive_identify(right_inputs, message, right_results);
        }
    }
}
